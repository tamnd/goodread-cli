...
# ca-certificates for HTTPS to www.goodreads.com; tzdata for sane timestamps.
RUN apk add --no-cache ca-certificates tzdata \
 && adduser -D -H -u 10001 goodread \
 && mkdir -p /data \
 && chown goodread:goodread /data

COPY $TARGETPLATFORM/goodread /usr/bin/goodread
...
#
#   docker run -v ~/data/goodread:/data ghcr.io/tamnd/goodread search dune --books
ENV GOODREAD_DATA_DIR=/data
VOLUME ["/data"]

ENTRYPOINT ["/usr/bin/goodread"]
